...
export function colOf(...args: [Bead, Map<string, Bead>]): string | null {
  const [bead, index] = args;
...
function byUpdatedThenPriority(pair: readonly [Bead, Bead]): number {
...
function byPriorityThenUpdated(pair: readonly [Bead, Bead]): number {
  const [first, second] = pair;
  return first.priority - second.priority || byUpdatedThenPriority(pair);
}

/** Sort cards according to the board's explicit display mode. */
export function sortBoardCards(
  ...args: [Bead[], BoardSortMode, string[]?]
): Bead[] {
  const [cards, mode, order] = args;
  const rank = new Map<string, number>();
  for (const id of order ?? []) rank.set(id, rank.size);

  return [...cards].sort((...pair: [Bead, Bead]) => {
    const [cardA, cardB] = pair;
    if (mode === "updated") return byUpdatedThenPriority(pair);
    if (mode === "priority") return byPriorityThenUpdated(pair);

    const rankA = rank.get(cardA.id) ?? Number.POSITIVE_INFINITY;
    const rankB = rank.get(cardB.id) ?? Number.POSITIVE_INFINITY;
    if (rankA !== rankB) return rankA - rankB;
    return byPriorityThenUpdated(pair);
  });
}

/** Sort by saved manual order, with a deterministic fallback for unranked cards. */
export function sortByOrder(...args: [Bead[], string[]?]): Bead[] {
  const [manualCards, manualOrder] = args;
  return sortBoardCards(manualCards, "manual", manualOrder);
}
